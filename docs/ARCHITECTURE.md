...
- **v0.3 — Universal proxy mode.** In progress. Built and tested: the proxy core for OpenAI chat-completions AND Anthropic Messages (all-or-nothing deny, buffered SSE replay, fail-closed on upstream errors, ungated POSTs refused), plus the localhost approval web UI (verified in a real browser). Verified end-to-end against OpenClaw 2026.6.11 with the plugin uninstalled, on Gemini 2.5 Flash: benign read flowed, `secrets.env` read was hard-denied at the proxy and reported to the agent. Remaining for the release: version bump and audit chain-head checkpointing to the relay (the latter may slip to v0.4 — it needs the relay redeployed).
