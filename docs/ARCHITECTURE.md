...
- **v0.3 — Universal proxy mode.** In progress. Built and tested: the proxy core for OpenAI chat-completions AND Anthropic Messages (all-or-nothing deny, buffered SSE replay, fail-closed on upstream errors, ungated POSTs refused), plus the localhost approval web UI (verified in a real browser). The proxy's gating was verified against a real model (Qwen3.5-9B via Ollama) with no plugin — a `.env` tool call was blocked, a benign read passed. Remaining for the release: full agent-loop verification with a stronger tool-capable model (the local 9B is too weak for OpenClaw's harness), and audit chain-head checkpointing to the relay.
